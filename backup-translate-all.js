...
const translateAllLocales = async () => {
    const mainAsset = await shopify.asset.get(process.env.THEME_ID, { 'asset[key]': "locales/en.default.json" });
    // fs.writeFileSync(path.join(__dirname, `./templates/in/${mainAsset.key}`), JSON.stringify(source, null, '\t'));
    const assets = await shopify.asset.list(process.env.THEME_ID);
    ...
    locales.map(async ({ key }) => {
        ...
        const targetAsset = await shopify.asset.get(process.env.THEME_ID, { 'asset[key]': `${key}` });
